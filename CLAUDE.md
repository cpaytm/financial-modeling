...
이 레포의 모든 HTML 산출물은 [docs/design_system.md](docs/design_system.md)와 [ds/tokens.js](ds/tokens.js)를 따른다.

### 절대 규칙

- **색상**: [ds/tokens.js](ds/tokens.js)의 토큰만 사용. 임의 hex 금지.
...
2. 9개 블록만 회사 데이터로 채움 ([framework/README.md](framework/README.md) 참조)
3. 엔진·UI 코드는 손대지 않음
...
임의로 만들지 말 것. 사용자에게 추가 요청 안내. 추가 시 절차는 [docs/design_system.md](docs/design_system.md) "DS에 없는 컴포넌트가 필요할 때" 참조.

---

## 방법론

추정 모델링 분석 작업 시 [docs/modeling_framework.md](docs/modeling_framework.md) Part I~II 준수.
...
[docs/modeling_framework.md](docs/modeling_framework.md) §2.3 / §12.
...
- `examples/getcha-fy26/` — 레거시 참조 구현. DS 미적용. **수정 금지** (역사적 기록).
...
- [ ] 엑셀 산출물이면 메타데이터 임베드했는가? ([modeling_framework.md](docs/modeling_framework.md) §18)
